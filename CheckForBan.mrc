...
alias Chkban { 
  if (!$1) { echo -a Use this format. /chkban Nameofuser!Ident@Host | halt }
  set %Ban.Scanning on
  set %Host $1
  mode $chan b
}
...
raw 367:*:{
  if (%Ban.Scanning) {
    if ($3 iswm %Host) { set %ban %ban $3 }
    if (($left($3,3) == ~n:) || ($left($3,3) == ~q:) && ($right($3,-3) iswm %host)) { set %ban %ban $3 }
    if ($left($3,3) == ~c:) { set %cban %cban $3 }
  }
}

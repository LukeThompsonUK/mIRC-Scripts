...
on $*:TEXT:/^[!@.]Temp\s([FC])\s(-?\d+)$/Si:#:{
  ...
  if ($regml(2) isnum) {
    if (F == $regml(1)) {
      msg $chan 07 $+ $regml(2) in Fahrenheit is:
      msg $chan 07 $+ $calc(($regml(2) - 32) * 5/9) in Celsius.
    }
    elseif (C == $regml(1)) {
      msg $chan 07 $+ $regml(2) in Celsius is:
      msg $chan 07 $+ $calc($regml(2) * (9/5) + 32) in Fahrenheit.
    }
  }
}

...
on $*:TEXT:/^!(s)?trivia$/Si:#Trivia:{
  if ((!trivia isin $1) && (!%Trivia)) { 
    set %trivia ON 
    setnewq 
  }
  elseif (!trivia isin $1) { 
    .msg $chan There is currently a Trivia game running. Type !Question to see the current question. 
  }
...
on $*:TEXT:/^!rank(\s.+)?$/Si:#Trivia:{
  if (!$2) {
    ...
    if (!$readini(TriviaScores.ini,n,$network,%nick)) { 
      .msg #Trivia $nick has 0 points. 
    }
    else { 
      .msg $chan $nick has $readini(TriviaScores.ini,n,$network,%nick) $iif($readini(TriviaScores.ini,n,$network,%nick) == 1,Point!,Points!) 
    }
  }
  else { 
    ...
    if (!$readini(TriviaScores.ini,n,$network,%nick)) { 
      ...
      .msg $chan $2 has $readini(TriviaScores.ini,n,$network,%nick) $iif($readini(TriviaScores.ini,n,$network,%nick) == 1,Point!,Points!) 
    }
  }
}
...
on $*:TEXT:/^!Q(uestion)?$/Si:#Trivia:{
  if (%NextQ) {
    if (!%Question.Spam) { 
      .msg $chan The current question is: %NextQ
      .msg $chan The answer space is: $regsubex(%ans,/\S/g,*) 
    }

    inc -u10 %Question.Spam
  }
  else { 
    .msg $chan The Trivia game is currently off or is waiting for a new question to be set. If a new question as not been set within 5 seconds; type 7!Trivia to start a new game. 
  }
}
...
on $*:TEXT:/^!h(int)?$/Si:#Trivia:{
  ...
  if (%hint < 4) { 
    var %Len $floor($calc($Len(%Ans) *(.75/ %hint))) 
  }
  else { 
...
on *:TEXT:*:#Trivia:{
  if ($istok($1-,%ans,42)) { 
    ...
    if ($readini(TriviaScores.ini,n,$network,%nick) == 500) { 
...
alias -l setnewq {
  ...
  if ($gettok(%Next,1,42) == Scramble) { 
    set %NextQ Unscramble the following: $scramble($gettok(%Next,2,42)) 
  }
  else { 
    set %NextQ $gettok(%Next,1,42) 
  }
...
alias -l scramble {
  ...
  while (%i <= $0) {
    ...
    while (%word != $null) { 
      var %rand = $rand(1, $len(%word)), %temp.smbl = %temp.smbl $+ $mid(%word, %rand, 1), %word = $left(%word, $calc(%rand - 1)) $+ $right(%word, $calc(-1 * %rand)) 
    }
...
on *:disconnect:{ 
  ...
  echo -a It seems we've disconnected from $network $+ . Stopping the Trivia game. 
}

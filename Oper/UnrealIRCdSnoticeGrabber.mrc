...
on $^*:Snotice:/Notice\s--\s(\S+)\s\((\S+)\)\shas\s(?:been\sforced\sto\s)?change(?:d)?\shis.her\snickname\sto\s(\S+)/Si:{
  if (!$window($($+(@NickChanges.,$network),2))) { 
    window -nz $($+(@NickChanges.,$network),2) 
  }
...
on $^*:Snotice:/HelpOp\s--\sfrom\s(\S+)\s(\(HelpOp\):\s)?(?:\(Local\):\s)?(.+)/Si:{
  ...
  if (!$window($($+(@HelpOp.,$network),2))) { 
    window -nezg1 $($+(@HelpOp.,$network),2) 
  }
...
on $^*:Snotice:/NetAdmin.Chat\s--\sfrom\s(\S+):\s(.+)/Si:{
  if (!$window($($+(@NaChat.,$network),2))) { 
    window -nezg1 $($+(@NaChat.,$network),2) 
  }
...
on $^*:Snotice:/AdminChat\s--\sfrom\s(\S+):\s(.+)/Si:{
  if (!$window($($+(@Adminchat.,$network),2))) { 
    window -nezg1 $($+(@AdminChat.,$network),2) 
  }
...
on $^*:Snotice:/ChatOps\s--\sfrom\s(\S+):\s(.+)/Si:{
  if (!$window($($+(@ChatOPS.,$network),2))) { 
    window -nezg1 $($+(@ChatOPS.,$network),2) 
  }

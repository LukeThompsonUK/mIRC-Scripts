...
on *:SockOpen:RandQuote:{
  if (%Quote == BASH) { 
    sockwrite -nt $sockname GET /lookup.php?type=bash 
  }

  if (%Quote == QDB) { 
    sockwrite -nt $sockname GET /lookup.php?type=qdb 
  }
...
on *:SockRead:RandQuote: {
  ...
  if (START isin %RandQuote) { 
    %show - 
  }
  
  if (Quote: isin %RandQuote) { 
    if ($right(%RandQuote,10) == [Bash.org]) 
    { 
      %show $left(%RandQuote,-10) 
    }
    elseif ($right(%RandQuote,8) == [QDB.US]) { 
      %show $left(%RandQuote,-8) 
    }
    else { 
      %show %RandQuote 
    }

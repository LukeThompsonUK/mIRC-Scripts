F12 {
  ...
  if (%bc.Show = Msg) { set %Ac.Show Msg $active }
  elseif (%bc.show = Echo) { Set %Ac.Show echo -a }
  else { Halt }
  var %z = 1, %i = 1, %b = 0
  ...
  while (%i <= $scon(0)) {
    scon %i {
      while (%z <= $chan(0)) {
        if ($left($nick($chan(%z),$me).pnick,1) == +) { inc %Voiced.Chans | set %Voiced.Name %Voiced.Name + $+ $chan(%z) }
        elseif ($left($nick($chan(%z),$me).pnick,1) == %) { inc %Halfed.Chans | set %Halfed.Name %Halfed.Name $eval(%,0) $+ $chan(%z) }
        elseif ($left($nick($chan(%z),$me).pnick,1) == @) { inc %Oped.Chans | set %Oped.Name %Oped.Name @ $+ $chan(%z) }
        elseif ($left($nick($chan(%z),$me).pnick,1) == &) { inc %Admin.Chans | set %Admin.Name %Admin.Name & $+ $chan(%z) }
        elseif ($left($nick($chan(%z),$me).pnick,1) == ~) { inc %Owner.Chans | set %Owner.Name %Owner.Name ~ $+ $chan(%z) }
        else { inc %Reg.chans | set %reg.name %reg.name $chan(%z) }
        inc %z
      }
      var %c $calc(%z -1)
      var %d $calc(%b + %c)
